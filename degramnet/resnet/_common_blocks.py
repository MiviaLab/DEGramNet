...
def ChannelECA(b=1, 
    ...
    def layer(input_tensor):
        ...
        t = int(np.ceil(np.log2(channels)+b/gamma)) 
        eca_k_size =  t if t % 2 else t+1

        x = input_tensor

        # squeeze and excitation block in PyTorch style with
        x = layers.GlobalAveragePooling2D()(x)
        x = layers.Lambda(expand_dims_1D, arguments={'channels_axis': channels_axis})(x)
        x = layers.Conv1D(1, eca_k_size, kernel_initializer='he_uniform',padding='same')(x)
        x = layers.Activation('sigmoid')(x)
        x = layers.Lambda(expand_dims_1D2D, arguments={'channels_axis': channels_axis})(x)
